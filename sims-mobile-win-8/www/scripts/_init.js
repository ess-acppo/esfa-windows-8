...
$(document).on('click', '#settings', function (e) {
    $.ajax({
        ...
        if (resSettings.settings.device.debugMode === 0) {
            $('#form3').find('input[id="debugMode"]').prop("checked", false);
            $('#showPayloads').addClass('hide');
        }
        if (resSettings.settings.device.debugMode === 1) {
            $('#form3').find('input[id="debugMode"]').prop("checked", true);
...
$(document).on('click', '.obsForm', function (e) {
    ...
    $(this).find('input[type="radio"].minimal').prop("checked", true);
});
...
function exportObservationsToCSV() {
    ...
    var flatJSON = data.map(function (record) { return flatten(record, {}, ''); });
...
function processzipWin(zipSource, destination) {
    ...
    //$.growl({ title: "", message: filename, location: "tc", size: "large" });
    //$.growl({ title: "", message: extracted, location: "tc", size: "large" });
    Zeep.unzip({
        from: zipSource.toLowerCase(),
        to: extracted.toLowerCase()
    }, function () {
